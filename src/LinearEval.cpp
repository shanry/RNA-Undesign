 ...
 *LinearFoldEval.cpp*
 ...
 author: Tianshuo Zhou (based on code by He Zhang)
 edited by: 09/2023
*/
...
long eval(string seq, string ref, bool is_verbose, int dangle_model) {
    ...
    vector<tuple<int, int, long>> xleft2triple;
    // vector<tuple<int, int, long>> ml_triples;
    map<int, long> ml2score;
    eval_nucs.clear();
    ...
    for (int j=0; j<seq_length; j++) {
        ...
        if (ref[j] == '.') {
            ...
            if (page == 0) { // hairpin
                ...
                if (is_verbose)
                    printf("> Hairpin loop ( %d, %d) %c%c : %.2f\n", i, j, seq[i], seq[j], newscore / -100.0);
                ...
                if (is_verbose)
                    printf("> Interior loop ( %d, %d) %c%c; ( %d, %d) %c%c : %.2f\n", i, j, seq[i], seq[j], p, q, seq[p],seq[q], newscore / -100.0);
                total_energy += newscore;
            }
                ...
                if (is_verbose)
                    printf("> Multi loop ( %d, %d) %c%c : %.2f\n", i, j, seq[i], seq[j], multi_score / -100.0);
                total_energy += multi_score;
                // multi_pair[i] = j;
                multi_left.push_back(i);
                ml2score[i] = multi_score;
                // ml_triples.push_back(make_tuple(i, j, multi_score));
            ...
            if (stk.empty()) {
                ...
                long external_score = - v_score_external_paired(k+1, j, nuck, nuck1,
                                                            nucj, nucj1, seq_length, dangle_model);
                external_energy +=  external_score;
                xleft2triple.push_back(make_tuple(i, j, external_score));
                // external_energy += 0; currently external unpaired is 0
                if (is_verbose)
                    printf("> External loop ( %d, %d) %c%c : %.2f\n", i, j, seq[i], seq[j], external_score / -100.0);
            }
        }
    }
    ...
    if (is_verbose and multi_left.size()){
        ...
        for(auto left: multi_left){
            printf("Multi outside # (%d, %d) : %.2f \n", left, pairs[left], ml2score[left]/-100.0);
            for(auto triple : mleft2triple[left])
                printf("Multi inside # ( %d, %d) : %.2f\n", get<0>(triple), get<1>(triple), get<2>(triple) / -100.0);
        }
    }
    if (is_verbose and xleft2triple.size())
        for(auto triple: xleft2triple)
            printf("External # ( %d, %d) : %.2f\n", get<0>(triple), get<1>(triple), get<2>(triple) / -100.0);
...
int main(int argc, char* argv[]){
    ...
    string seq;
    string ref;
    while(cin >> seq && cin >> ref){
        cout << seq << endl;
        cout << ref << endl;
        bool verbose = true;
        int dangle = 2;
        long energy = eval(seq, ref, verbose, dangle);
